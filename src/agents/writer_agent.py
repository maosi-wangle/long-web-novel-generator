...
class WriterAgent:
    ...
    def write_chapter(
        ...
        *,
        ...
        retrieved_context = self._merge_retrieved_context(detail_outline, rag_result)
        messages = [
            {"role": "system", "content": self.prompt_template},
            {
                ...
                "content": (
                    ...
                    f"局部写作包：\n{self._writer_context(detail_outline, retrieved_context)}"
                ),
            },
        ]
    ...
    @staticmethod
    def _merge_retrieved_context(detail_outline: DetailOutline, rag_result: RagSearchResult) -> list[str]:
        merged: list[str] = []
        seen: set[str] = set()

        for item in detail_outline.writer_packet.retrieved_context:
        ...
        for hit in rag_result.hits:
            normalized = f"{hit.chunk_id}: chapter_{hit.chapter_id} {hit.summary}"
    ...
    @staticmethod
    def _writer_context(detail_outline: DetailOutline, retrieved_context: list[str]) -> str:
        writer_packet_payload = detail_outline.writer_packet.model_dump(mode="json")
        writer_packet_payload["retrieved_context"] = retrieved_context
        payload = {
